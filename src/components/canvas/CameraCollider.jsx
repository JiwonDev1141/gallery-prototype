...
function CameraCollider() {
  ...
  const { forward, backward, left, right, jump, leftRotate, rightRotate } =
    usePersonControl();
  const { nodes, materials, animations } = useGLTF(
    "/models/core_from_portal_2.glb"
  );
  ...
  useFrame(() => {
    ...
    const newRotateZ =
      (Number(leftRotate) - Number(rightRotate)) / Math.PI + rotationZ;
    newRotateZ ? setRotationZ(newRotateZ) : console.log();
  });
