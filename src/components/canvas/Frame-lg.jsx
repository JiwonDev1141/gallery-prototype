...
const FrameLg = ({ cameraRef, scale = 1, position = [0, 0, 0], rotation = [0,0,0], showPop, setShowPop, img = "/bird.jpg", img2 = "/logo512.png" }) => {
    ...
    return (
        <>
            ...
            <primitive object={cloned} onClick={() => showImgPop()} onPointerOver={() => window.document.body.style.cursor="pointer"} onPointerOut={ () => window.document.body.style.cursor="default"} scale={scale} rotation={rotation} position={[position[0], position[1], position[2]+5]}/>
            <mesh position={[position[0] -0.1, position[1] + (scale * 0.95), position[2] + 5]} rotation={[rotation[0],rotation[1] + 4.7, rotation[2]]} scale={[scale * 1.3, scale * 1.6, 1]}>
                <planeGeometry attach={"geometry"}  />
